...
## 需要「人」的时刻（右上角横幅 + 声音，正文直接附账密）

提醒走 macOS 通知横幅（不弹框、不抢焦点），正文直接附管理端账密（内网使用，
通知不离开本机）。**状态刚变为异常时系统会自动打开对应页面一次**（如扫码页），
之后每 30 分钟只发横幅提醒、不反复抢焦点；恢复正常后自动停止提醒。

- **微信读书授权失效**（约 24h 过期或换 IP）：自动打开 `:8001/weread`，扫码即恢复。
  影响文章正文，不影响账号添加。
...
> 系统设置 → 通知 → 「Script Editor」（脚本编辑器）→ 通知样式选 **「提醒」(Alerts)**

（werss 的横幅由 osascript 发出，系统里登记在 Script Editor 名下；找不到该条目时，
先等 werss 发过一次提醒，它就会出现。）
