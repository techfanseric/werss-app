...
## 需要「人」的时刻（都会收到 Mac 系统通知）

- **微信读书授权失效**（约 24h 过期或换 IP）：通知 + 自动打开 `:8001/weread` 扫码页 +
  二维码截图存 `batch/qr_need_scan.png`；未处理每 30 分钟再提醒。影响文章正文，不影响账号添加。
- **公众号平台登录过期**（当前约 09-27 到期）：换公众号账号登录可得新的搜索配额。
...
## 配置（`config.env`，唯一配置来源）

端口（默认 8001）、容器名、镜像（默认锁定与原系统同一构建）、管理端账号密码、
采集节奏（防风控参数，与原系统一致，**不建议调快**）。
