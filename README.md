...
## 需要「人」的时刻（右上角横幅 + 声音，**点击通知直达**，正文附账密）

提醒走 macOS 通知横幅（不弹框、不抢焦点、不自动开浏览器）。**需要你操作时，
点一下横幅本体即可**——系统会用 ego 浏览器自动登录并直达对应页面（如扫码页）
再把窗口带到前台；登录账密直接写在通知正文里（内网使用，通知不离开本机）。
未处理时每 30 分钟重新提醒（同组横幅自动替换，不堆积）；恢复正常后自动停止。

- **微信读书授权失效**（约 24h 过期或换 IP）：点击通知 → 直达扫码页。影响文章
  正文，不影响账号添加。
- **公众号平台登录过期**：换公众号账号登录可得新的搜索配额。
- **Docker 未恢复 / 应用未就绪 / 连续 3 次保活失败**：横幅提醒人工处理。
- **限流**：runner 自动冷却 40 分钟，金丝雀词轮换探测，无需人工。

> 「点击直达」依赖 `terminal-notifier`（安装器会用 brew 自动装；首次弹出通知
> 权限请求时点「允许」）。没有它时降级为：状态刚失效时自动打开一次页面。

### 一次性设置：让横幅不自动收起（推荐）

macOS 横幅默认 ~5 秒收起进通知中心。要让 werss 的横幅**停留在屏幕右上角直到
你手动关掉**，做一次系统设置（之后所有 werss 横幅都是常驻样式）：

> 系统设置 → 通知 → **terminal-notifier**（或降级时的 Script Editor）→ 通知样式选 **「提醒」(Alerts)**
