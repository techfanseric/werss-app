...
## 日常看状态：菜单栏 werss 图标（推荐）

右上角常驻 `werss ✓ / ⚠️ / ✕`（正常/待办/故障），每 30 秒自动刷新，点开即见：
状态总览（Docker/容器/runner/待采队列/订阅/文章/授权）与**待办直达**——
「微信读书待扫码：点此直达（自动登录）」「系统故障：点此立即保活修复」，
以及管理页/立即保活/交接导出导入。崩溃自动重启（launchd 保活），开机自启。
（重建：`bash bin/build_menubar.sh`，改 src/menubar_app.swift 后）

## 招聘帖审核与发布（核心业务闭环）

采集的最终目的：从订阅公众号里持续发现**招聘帖**（校招/秋招/社招/内推/宣讲会等），
人工审核后一键发布到接口。配置在 `config.env`（`MINIMAX_API_KEY` / `JOBS_PUBLISH_URL`）。

```
keepalive 每 30 分钟 → bin/jobs.py scan
  ├─ 规则粗筛：招聘号(名称含「招聘」)全部文章 + 标题关键词(招聘/秋招/校招/社招/内推/实习/招贤/纳士/网申/双选会/宣讲会)
  ├─ MiniMax 精判：区分真招聘帖 vs 团建/节日/企业文化噪音；抽取类型/届别/主体/岗位；旧届别标「旧文」
  └─ 写状态库 data/jobs_review.json（随备份包交接，发布记录不丢）
菜单栏 →「招聘帖审核（待审 N）」→ 专属审核窗口
  ├─ 按发现日期倒序分组（最新在最上，分组头显示篇数/旧文数），状态筛选：待审/已发布/已忽略/全部
  ├─ 搜索框（公司/标题/岗位）、「隐藏旧文」开关（待审页过滤回填旧帖）、⌘R 刷新、窗口位置大小自动记忆
  ├─ 单篇：发布（调 JOBS_PUBLISH_URL 接口+标记）/ 不发布（永久忽略）/ 撤回；标题点击或双击行打开原文，
  │       右键菜单（打开原文/拷贝链接/发布/不发布）；操作后本地即时更新不闪烁
  └─ 按日：「一键发布」带确认弹窗（含旧文警示）+ 逐篇进度 + 成功/失败汇总
有新招聘帖 → 菜单项加 ● 前缀加粗 + macOS 系统通知（点通知直达窗口）；窗口打开期间每 60 秒静默刷新
```

- **防重复**：发布状态落在状态库，已发布的篇目不会再出现在待审；发布接口失败不改状态可重试。
- **时效性**：文章按发现日期分组（日常增量发现时间≈真实发文时间，延迟约 0.5~2 小时）。
  注意：库内 `publish_time` 是抓取入库时间，非公众号真实发文时间；回填的旧文靠届别识别。
- 未配置 `JOBS_PUBLISH_URL` 时点「发布」仅本地标记（联调安全）；配置后 POST 的 JSON 载荷
  见 `bin/jobs.py build_payload`（含标题/原文链接/公众号/公司/类型/届别/岗位/时间）。
- 手动：`python3 bin/jobs.py scan`（扫描）/ `stats`（计数）/ `list --json`（列表）。


## 需要「人」的时刻（通知只做提示，操作走控制台/菜单栏）
