...
## 需要「人」的时刻（都会弹**不消失的对话框**，点按钮直达）

提醒为 macOS 模态对话框（不会像横幅一样自动收起），带声音；正文直接附管理端
账密（内网使用，通知不离开本机），点「打开扫码页」直达页面：

- **微信读书授权失效**（约 24h 过期或换 IP）：影响文章正文，不影响账号添加；
  二维码截图另存 `batch/qr_need_scan.png`。
- **公众号平台登录过期**：换公众号账号登录可得新的搜索配额。
- **保活连续 3 次失败 / Docker 未恢复 / 安装受阻**：弹对话框等人工处理。
