...
if ! docker_ok; then
  ...
  docker_ok && echo "Docker 就绪 ✓" || { notify "安装受阻" "Docker 引擎 10 分钟未就绪，请完成 Docker 首次启动设置后重新双击 安装.command"; exit 1; }
else
  echo "Docker 运行中 ✓"
fi
...
if ! command -v ego-browser >/dev/null 2>&1; then
  ...
  command -v ego-browser >/dev/null 2>&1 && echo "ego-browser 命令就绪 ✓" || { notify "安装受阻" "ego lite onboarding 未完成，完成后重新双击 安装.command"; exit 1; }
else
...
if [ -f "$WERSS_ROOT/backups/werss-image.tar.gz" ]; then
  ...
  docker pull "$WERSS_IMAGE" && echo "镜像就绪 ✓" || { notify "镜像拉取失败" "请检查网络后重跑 安装.command"; exit 1; }
fi
